...
# monthly payment is a multiple of $0.01). Does your code still work?
...
# each submission is allowed, so your observations from running this experiment
# on the grading system might be limited to an error message complaining about
# too much time taken.)
#
...
# a reasonable lower bound for this payment value? $0 is the obvious answer,
...
paymentLo = balance / 12
paymentHi = balance * (1 + monthlyInterestRate) ** 12 / 12
tolerance = 0.01
while True:
    tempBalance = balance
    payment = round((paymentHi + paymentLo) / 2, 2)
    ...
    if tempBalance > 0 and round(payment - paymentLo, 2) > tolerance:
        paymentLo = payment
    elif tempBalance < 0 and round(payment - paymentLo, 2) > tolerance:
        paymentHi = payment
    else:
